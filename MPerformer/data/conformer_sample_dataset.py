...
class ConformerSampleDataset(BaseWrapperDataset):
    def __init__(self, dataset, seed, atoms, coordinates, target, atom_H_num, bond, noise_weight, noise_radio):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        with data_utils.numpy_seed(self.seed, epoch, index):
            ...
            noise_coord = np.random.randn(self.dataset[index][self.coordinates][sample_idx].shape[0],3) * self.noise_weight
            noise_prob = np.random.rand()
            
        if noise_prob > 0.2:
            coordinates = self.dataset[index][self.coordinates][sample_idx] + noise_coord
        else:  
            coordinates = self.dataset[index][self.coordinates][sample_idx] 
                
        target = self.dataset[index][self.target]
        atom_H_num = self.dataset[index][self.atom_H_num]
        bond = self.dataset[index][self.bond]
        
        target_coordinates = self.dataset[index][self.coordinates][sample_idx] 

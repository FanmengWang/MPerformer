...
from MPerformer.data.tta_dataset import TTADataset
...
@register_task("MPerformer")
class MPerformerTask(UnicoreTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            default=1.0,
            type=float,
            help="weight of noise added to coordinates",
        )
        parser.add_argument(
            "--noise-radio",
            default=2,
            type=int,
            help="radio of atoms added noise",
        )
        parser.add_argument(
            "--noise-valid",
            default=1,
            type=int,
            help="whether add noise on valid",
        )
        parser.add_argument(
            "--remove-hydrogen",
            action="store_true",
    ...
    def __init__(self, args, dictionary):
        ...
        self.bond_pad_idx = 6 # 0: no bond, 1: single bond, 2: double bond, 3: triple bond, 4: aromatic bond, 5:other_bond, 6: pad
    ...
    def load_dataset(self, split, **kwargs):
        ...
        if split == "train":
            ID_dataset = KeyDataset(dataset, "data")
            dataset = ConformerSampleDataset(
                dataset, self.args.seed, "atoms", "coordinates", "target", "atom_H_num", "bond", self.args.noise_weight, self.args.noise_radio
            )

        else:
            ID_dataset = KeyDataset(dataset, "data")
            dataset = TTADataset(
                dataset, self.args.seed, "atoms", "coordinates", "target", "atom_H_num", "bond", self.args.conf_size, self.args.noise_weight, self.args.noise_radio, self.args.noise_valid
            )
        ...
        dataset = NormalizeDataset(dataset, "coordinates", "target_coordinates", normalize_coord=True)
        ...
        tgt_coord_dataset = KeyDataset(dataset, "target_coordinates")

        tgt_dataset = KeyDataset(dataset, "target")
        
        atom_H_num_dataset = KeyDataset(dataset, "atom_H_num")
        
        bond_dataset = KeyDataset(dataset, 'bond')
        ...
        tgt_dataset = FromNumpyDataset(tgt_dataset)
        tgt_dataset = PrependAndAppend(tgt_dataset, self.dictionary.pad(), self.dictionary.pad())
        
        atom_H_num_dataset = FromNumpyDataset(atom_H_num_dataset)
        atom_H_num_dataset = PrependAndAppend(atom_H_num_dataset, self.atom_H_pad_idx, self.atom_H_pad_idx)
        
        tgt_bond_dataset = PrependAndAppend2DDataset(FromNumpyDataset(bond_dataset), self.bond_pad_idx)
        ...
        nest_dataset = NestedDictionaryDataset(
            {
                ...
                "target": {
                    ...
                    "atom_H_num_target": RightPadDataset(atom_H_num_dataset, pad_idx=self.atom_H_pad_idx),
                    'bond_target': RightPadDataset2D(tgt_bond_dataset, pad_idx=self.bond_pad_idx),
                    "distance_target": RightPadDataset2D(tgt_distance_dataset, pad_idx=0),
                    "coord_target": RightPadDatasetCoord(tgt_coord_dataset, pad_idx=0),
                },   
                
                "id_name": RawArrayDataset(ID_dataset),             
            },
        )
